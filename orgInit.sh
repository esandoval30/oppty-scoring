...
sfdx force:data:bulk:upsert -s Contact -w 3 -f data/AEs-1.csv -i External_ID__c
#Bulk Load ALL Client (Accounts) linked to their corresponding AE
sfdx force:data:bulk:upsert -s Account -w 3 -f data/Account.csv -i External_ID__c
#Bulk Load ALL Opportunities
sfdx force:data:bulk:upsert -s Opportunity -w 3 -f data/AllOpportunities.csv -i External_ID__c
